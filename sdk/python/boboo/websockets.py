...
from .exceptions import BobooRequestException
from . client import BobooClient


class BobooClientProtocol(WebSocketClientProtocol):
...
class BobooReconnectingClientFactory(ReconnectingClientFactory):
...
class BobooClientFactory(WebSocketClientFactory, BobooReconnectingClientFactory):
    ...
    def buildProtocol(self, addr):
        return BobooClientProtocol(self, payload=self.payload)


class BobooSocketManager(threading.Thread):

    def __init__(self, api_key='', secret='', entry_point='wss://wsapi.boboo.com/openapi/', auth=True, rest_entry_point='https://api.boboo.com/openapi/'):
        ...
        if auth:
            self._client = BobooClient(api_key=self._api_key, secret=self._secret, entry_point=rest_entry_point) if api_key and secret else None
    ...
    def _start_socket(self, id_, path, payload, callback):
        ...
        factory = BobooClientFactory(factory_url, useragent='Boboo-P 1.0', payload=payload)
        factory.base_client = self
        factory.protocol = BobooClientProtocol
    ...
    def _start_auth_socket(self, id_, payload, callback):
        ...
        if not listen_key:
            raise BobooRequestException('Get listen key failure.')
...
class BobooWss(BobooSocketManager):
